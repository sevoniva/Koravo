...
All queries are scoped by the current `X-Koravo-Tenant-Id` and login session. The console `Audit Logs` page supports the same filters and can open a single event to inspect metadata and formatted redacted details.
