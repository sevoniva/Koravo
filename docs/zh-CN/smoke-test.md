...
- 任务详情办理：通过。`/tasks/e2267237-6272-11f1-bd58-c232b2af3b82` 渲染任务头、状态标签、绑定表单和折叠详情数据；清空必填项后提交被前端拦截，审批动作切换可同步变量和意见，控制台无错误。
...
- 流程实例详情：通过。`/process-instances/cb7fb909-625a-11f1-ab24-c232b2af3b82` 渲染实例状态、流程图、当前任务、变量摘要和审计摘要；原始脱敏数据收进高级详情；接口返回 200 且控制台无错误。
