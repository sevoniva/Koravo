...
- 是否已有可部署流程资产；开发环境缺失时可在「开发资产检查」补齐
- 流程模型是否已部署
- 流程定义 Key 是否为 `multiAcceptance`
- 启动表单必填字段是否已填写
- 业务处理人和财务复核人是否为有效用户，例如 `manager`、`finance`

## 我的待办为空

检查当前登录成员和职责。多人验收流程会按业务处理人和财务复核人分配待办，可在「组织权限」确认成员职责后查看「我的待办」。
