...
- 首页首屏已压缩为运行上下文、快捷动作和核心指标，不再用租户、用户、请求追踪等上下文字段占用指标卡。
- 首页指标和最近审计已去除后端枚举、Flowable 术语和英文死信文案，默认展示中文业务动作与对象。
- 顶栏、首页和系统设置已统一使用“请求追踪”，不再把请求上下文作为技术 ID 展示。
- 快速开始改为可操作步骤，支持初始化演示数据和启动请假流程。
- 快速开始默认展示请假申请摘要，原始启动变量移入高级详情。
- 快速开始默认不再展示流程 Key、流程定义 ID 和表单 UUID，改为流程准备状态；系统设置隐藏演示接口文案和完整响应 ID。
- 流程模型列表改为业务摘要和本地关键词搜索，默认不再展示模型 Key、完整流程定义 ID 和演示说明。
- HTTP 连接器改为可部署、可启动、可查看日志和结果变量。
- 任务中心已支持关键词、状态和日期范围筛选。
- 任务详情默认展示任务节点、流程实例和流程业务标签，完整 ID 仅通过复制按钮保留。
- 表单管理已支持可视化字段编辑、表单预览和高级配置查看。
- 表单绑定已支持从 BPMN 自动加载任务节点，默认展示已有绑定对应的流程模型。
- 表单绑定默认展示流程、任务和表单的业务标签，模型 ID、流程定义和任务节点编码不再占据首屏。
- 审计日志已支持统一筛选、分页、详情查看和审计详情脱敏，默认使用对象和请求短追踪号。
...
- 流程实例列表和详情默认展示短追踪号、业务编号、业务流程和节点名称，完整 ID 仅通过复制按钮保留。
...
- 任务详情流程实例、流程定义、表单快照和操作记录已改为业务标签或短追踪号展示，完整值保留复制入口。
- 任务列表办理弹窗和任务详情办理区默认展示中文表单，表单数据和流程变量收进高级配置。
- 任务列表默认展示中文任务节点，我发起的流程显示短追踪号，办理弹窗不再展示表单技术标识。
- 表单设计器主工作区保留表单信息、字段、属性和预览，高级配置默认收起。
...
- 审计日志对象、请求、流程、模型和表单字段已改为业务标签或短追踪号展示，不再默认展示完整 UUID 和技术 Key。
- 流程模型页已移除主视图裸 JSON，部署结果、模型详情和校验结果改为中文详情区。
- 流程模型页默认展示流程名称、业务说明和短定义标签，完整编码仅通过复制按钮保留。
- 流程设计器模型列表、模型属性和节点属性已改为业务标签，不再默认展示模型 Key、节点 ID 和示例文案。
...
- 运维中心实例、连接器请求、失败任务和死信任务默认展示短追踪号，流程和连接器类型使用业务标签，完整值保留复制入口。
- 流程设计器移动端改为单列堆叠，避免三栏布局挤压。
- 设计器画布、侧栏和属性面板已补最小宽度和滚动约束，移动端单列、桌面三栏均不产生页面级横向溢出。
- HTTP 连接器页已移除首屏 `httpConnectorDemo`、空 JSON 和 ISO 时间，默认展示调用状态、健康检查摘要、当前任务和日志摘要；原始变量与日志移入高级详情。
- HTTP 连接器页操作已改为“准备流程”和“发起健康检查”，指标区改为紧凑展示，避免演示操作口吻占据首屏。
- 流程实例详情审计摘要已去除演示和 HTTP Connector 示例文案，统一展示基础数据、健康检查和 HTTP 调用。
