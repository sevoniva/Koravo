...
class ConnectorExecutionLogServiceTest {
    ...
    private final ConnectorExecutionLogService service = new ConnectorExecutionLogService(repository);

    @Test
    void recordSuccessRedactsSensitiveRequestData() {
        when(repository.save(any(KoConnectorExecutionLog.class))).thenAnswer(invocation -> invocation.getArgument(0));
