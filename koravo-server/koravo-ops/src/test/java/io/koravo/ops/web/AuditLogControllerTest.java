...
class AuditLogControllerTest {
    ...
    void queryPassesTimeRangeToService() {
        ...
        when(auditLogQueryService.query("admin", "TASK_COMPLETE", "TASK", startTime, endTime, 2, 10))
                .thenReturn(PageResult.of(List.of(), 0, 2, 10));

        var response = controller.query("admin", "TASK_COMPLETE", "TASK", startTime, endTime, 2, 10);

        assertThat(response.success()).isTrue();
        assertThat(response.data().page()).isEqualTo(2);
        verify(auditLogQueryService).query("admin", "TASK_COMPLETE", "TASK", startTime, endTime, 2, 10);
    }
}
